...
def load_data():
    ...
    """(img_zrm, label_zrm), (testX, testy) = (x_train, y_train),(x_test, y_test)
    for c in range(1, 10): 
        img_zrm_B = np.vstack(
    ...
    x_train, x_test, y_train, y_test = img_zrm_B, img_zrm_B_test, label_zrm_B, label_zrm_B_test"""
    rng = np.random.RandomState(20)
    random_unlabeled_points = rng.rand(y_train.shape[0]) < 0.36#change number of unlabeled picture
    print(y_train.shape)
